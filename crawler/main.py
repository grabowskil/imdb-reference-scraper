...
def imdbScraper(titleLink, wait_time=5):
    ...
    if r.status_code != 404:
        ...
        print("process: next title '" + title_str + "'")
        ...
        if acc_csv.inList(title_str) == False:
            print("process: writing in csv")
            acc_csv.writeCsv(title_str, div_list)
        else:
            print("process: known title, just parsing div_list")
        
        elapsed_time = time.time() - start_time
        if elapsed_time < wait_time:
            sleep_time = wait_time - elapsed_time
            print("process: need to wait {:.0} seconds".format(sleep_time))
